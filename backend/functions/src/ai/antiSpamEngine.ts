import { RawHazardInput, GeoCoordinates } from "../types/models";
...
const recentSubmissionsCache: RecentSubmission[] = [];

/**
 * Calculates distance in meters between two coordinates using Haversine formula
 */
function getDistanceMeters(c1: GeoCoordinates, c2: GeoCoordinates): number {
...
export function evaluateAntiSpam(
  ...
  const userSubmissions = recentSubmissionsCache.filter(
    (s) => s.reportedBy === report.reportedBy && currentTimeMs - s.timestamp < 15 * 60 * 1000 // 15 mins window
  );
 ...
 */
export function clearAntiSpamCache(): void {
