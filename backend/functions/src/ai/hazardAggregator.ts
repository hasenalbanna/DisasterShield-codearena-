...
import { evaluateImageAi } from "./imageAi";
import { evaluateWeatherAi } from "./weatherAi";
...
export async function runHazardAggregatorAi(
  ...
  const [imageRes, weatherRes, clusterRes, locationRes] = await Promise.all([
    ...
    evaluateWeatherAi(report),
    evaluateClusterAi(report, existingReportsInWard),
    evaluateLocationAi(report),
  ]);
