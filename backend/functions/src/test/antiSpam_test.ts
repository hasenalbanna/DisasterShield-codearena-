import { evaluateAntiSpam, clearAntiSpamCache } from "../ai/antiSpamEngine";
import { RawHazardInput } from "../types/models";

function createMockReport(overrides: Partial<RawHazardInput> = {}): RawHazardInput {
  return {
    hazardId: "hz_test_01",
...
export function runAntiSpamTests(): boolean {
  ...
  const res2 = evaluateAntiSpam(repDuplicate, 85);
  ...
  const res3 = evaluateAntiSpam(repFuture, 85);
  ...
  const res4 = evaluateAntiSpam(repLowTrust, 25); // Reputation 25/100
  ...
  clearAntiSpamCache();
  const now = Date.now();
  evaluateAntiSpam(createMockReport({ hazardId: "hz_sub_1" }), 80, now);
  evaluateAntiSpam(createMockReport({ hazardId: "hz_sub_2", coordinates: { latitude: 6.95, longitude: 79.88 } }), 80, now + 1000);
  evaluateAntiSpam(createMockReport({ hazardId: "hz_sub_3", coordinates: { latitude: 6.96, longitude: 79.89 } }), 80, now + 2000);
  const resRateLimit = evaluateAntiSpam(createMockReport({ hazardId: "hz_sub_4", coordinates: { latitude: 6.97, longitude: 79.90 } }), 80, now + 3000);
